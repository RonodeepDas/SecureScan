...
const SCORE_META = [
  ...
  { label: 'Strong',     color: 'bg-emerald-500', text: 'text-emerald-400',bars: 4 },
  { label: 'Very Strong',color: 'bg-cyan-500',    text: 'text-cyan-400',   bars: 5 },
]
...
function FeedbackList({ result }) {
  ...
  if (password.length < 12 && password.length >= 8)
    suggestions.push('Longer passwords (12+) are exponentially harder to crack')

  if (feedback.warning)
    suggestions.push(feedback.warning)
  ...
  if (suggestions.length === 0 && score >= 3)
    return <p className="text-emerald-400 text-xs mt-3">✓ No major weaknesses detected</p>
...
function HistoryTable({ rows }) {
  if (!rows.length) return (
    <p className="text-gray-500 text-sm text-center py-4">No history saved yet.</p>
  )
...
export default function PasswordChecker() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveMsg({ ok: true, text: 'Metadata saved (score + crack time only).' })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-4 border-l-4 border-indigo-500 text-sm text-gray-300">
        <span className="font-semibold text-indigo-400">🔐 Privacy guaranteed:</span>{' '}
        Your password is analyzed entirely in this tab using{' '}
        <code className="text-cyan-400 text-xs">zxcvbn</code>. It is never transmitted,
        stored, or logged — not even encrypted.
      </div>

      {/* Checker card */}
      <div className="glass-card p-6">
        <h2 className="text-lg font-bold mb-4 text-white">Password Strength Checker</h2>
            ...
            <div className="flex items-center justify-between">
              ...
              {crackTimeDisplay && (
                <span className="text-xs text-gray-400">
                  Crack time: <span className="text-gray-200 font-medium">{crackTimeDisplay}</span>
                </span>
              )}
            </div>
        ...
        {result && (
          <div className="mt-6 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <span className="text-xs text-gray-500">
              Saves: score ({result.score}/4) + crack-time estimate + timestamp only.
            </span>
          </div>
        )}

        {saveMsg && (
          <p className={`mt-3 text-xs font-medium ${saveMsg.ok ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      <div className="glass-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button id="toggle-pw-history" className="btn-secondary text-xs py-1.5 px-3" onClick={toggleHistory}>
